...
const ORGS: Org[] = [
  ...
  },
  {
    slug: "initech",
    name: "Initech",
    wsName: "Initech Sales",
    wsSlug: "initech-sales",
    ownerEmail: "owner@initech.test",
  },
];
...
async function ensureOrg(o: Org): Promise<{ tenantId: string; workspaceId: string }> {
  ...
  await db
    ...
    .values({ tenantId: t!.id, userId: ownerId, isTenantOwner: true, status: "active" });
  const [ws] = await db
    .insert(workspaces)
    .values({
      tenantId: t!.id,
...
async function main(): Promise<void> {
  ...
  console.log(
    `seed: acme=${acme.tenantId} globex=${globex.tenantId} initech=${initech.tenantId} multi=${multiId}`,
  );
