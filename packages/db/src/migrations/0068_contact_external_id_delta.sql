...
-- back by a flag.
INSERT INTO feature_flags (key, description, global_enabled, "default") VALUES ('delta_imports_enabled', 'Per-tenant rollout gate for P5 incremental/delta imports (import-and-data-model-redesign 08 §9; external_id upsert layer). OFF by default (fail-closed): while off every import keeps its shipped content-hash idempotent-skip + email→linkedin→sales-nav dedup ladder byte-identical and never reads or writes contacts.external_id. Effective only when the global DELTA_IMPORTS_ENABLED env kill-switch is also on AND a caller maps an externalId column with the per-import externalIdUpsert option; with all layers on, the caller-declared external_id becomes the TOP dedup rung (an incoming row resolves to the contact holding that external key before the email rung is consulted — Salesforce-style upsert-on-external-id) and is stamped onto newly created contacts. The row-grain content_hash skip (already shipped on source_imports) stays the internal delta mechanism for re-imports and is unaffected by this flag.', false, false) ON CONFLICT (key) DO NOTHING;
