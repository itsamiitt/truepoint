...
INSERT INTO feature_flags (key, description, global_enabled, "default") VALUES ('account_domains_dual_write', 'Per-tenant rollout gate for the account-domain dual-write (import-and-data-model-redesign 06 §1; S-A2). OFF by default (fail-closed): while off, account writers keep the shipped flat accounts.domain behavior and the account_domains child table stays unwritten. Effective only when the global ACCOUNT_DOMAINS_DUAL_WRITE env kill-switch is also on; with both on, writers maintain the child row plus the flat cache via applyAccountDomainWrite in one transaction. Also gates the S-A1/S-A3 backfill sweep.', false, false) ON CONFLICT (key) DO NOTHING;
