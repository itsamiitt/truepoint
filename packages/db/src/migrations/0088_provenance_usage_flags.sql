...
INSERT INTO feature_flags (key, description, global_enabled, "default") VALUES ('provenance_events', 'Per-tenant gate for the OVERLAY half of the provenance_event dual-write (08-architecture invariant 1; S-06). OFF by default, fail-closed: while off, contact/account writers keep their shipped field_provenance behaviour and append no events. Effective only with the PROVENANCE_EVENTS_ENABLED env switch also on. Layer-0 master_* events ride the env half ALONE (Layer 0 has no tenant_id to key on). field_provenance stays authoritative in Phase 1.', false, false) ON CONFLICT (key) DO NOTHING;
