...
INSERT INTO feature_flags (key, description, global_enabled, "default") VALUES ('scheduled_imports_enabled', 'Per-tenant rollout gate for scheduled imports (import-and-data-model-redesign 08 §9; P5). OFF by default (fail-closed): while off the scheduled-imports CRUD verbs 404 and the leader-locked sweep fires nothing for the tenant. Effective only when the global SCHEDULED_IMPORTS_ENABLED env kill-switch is also on AND the import_v2 dual gate is on for the tenant (a fired run rides the unified durable pipeline). Due schedules then submit an import_jobs row via submitCopyImport.', false, false) ON CONFLICT (key) DO NOTHING;
