...
--
-- SESSION PRUNING (RC#4, follow-up — NOT in this migration): user_sessions is append-only and never
-- pruned, so even the partial index below keeps growing with revoked rows. The pruning method
-- (sessionRepository.deleteExpired: DELETE WHERE revoked_at IS NOT NULL OR expires_at < now()) belongs in
-- packages/db/src/repositories/userRepository.ts, which is owned by a different unit in this batch — it is
-- intentionally NOT added here to avoid a cross-unit edit conflict. A scheduled worker (apps/workers) should
-- call it daily. Until then this index is partial-on-live-rows, so reads stay fast; only storage grows.
--
