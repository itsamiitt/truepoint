...
INSERT INTO feature_flags (key, description, global_enabled, "default") VALUES ('channels_dual_write', 'Per-tenant rollout gate for the multi-value channel dual-write (import-and-data-model-redesign 05; S-CH2). OFF by default (fail-closed): while off every email/phone writer keeps its shipped flat-column behavior byte-identical and the contact_emails/contact_phones child tables stay unwritten. Effective only when the global CHANNEL_DUAL_WRITE env kill-switch is also on; with both on, the import/enrichment/reveal/re-verification writers additionally maintain child rows via applyChannelWrite in the same transaction (CH-INV-1). Flat columns remain the source of truth until S-CH4.', false, false) ON CONFLICT (key) DO NOTHING;
