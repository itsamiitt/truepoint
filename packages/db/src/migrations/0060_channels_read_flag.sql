...
INSERT INTO feature_flags (key, description, global_enabled, "default") VALUES ('channels_read', 'Per-tenant rollout gate for the multi-value channel read cutover (import-and-data-model-redesign 05; S-CH4). OFF by default (fail-closed): while off, contact reads keep the shipped flat-column behavior and the child tables stay unread. Effective only when the global CHANNEL_READ_FROM_CHILD env kill-switch AND the full S-CH2 dual-write gate are also on — read implies dual-write. With all layers on, the child tables become the read truth. Flip only after backfill completeness = 0 and drift = 0.', false, false) ON CONFLICT (key) DO NOTHING;
