...
describe("compareRecords", () => {
  ...
  test("LinkedIn id: same → agree, different → disagree, missing → not_compared", () => {
    expect(
      byField(
        compareRecords({ linkedinPublicId: "li-1" }, { linkedinPublicId: "li-1" }),
        "linkedin",
      ).comparison,
    ).toBe("agree");
    expect(
      byField(
        compareRecords({ linkedinPublicId: "li-1" }, { linkedinPublicId: "li-2" }),
        "linkedin",
      ).comparison,
    ).toBe("disagree");
    expect(byField(compareRecords({ linkedinPublicId: "li-1" }, {}), "linkedin").comparison).toBe(
      "not_compared",
    );
  });
  ...
  test("email blind-index hex compares exactly (opaque; no plaintext)", () => {
    expect(
      byField(
        compareRecords({ emailBlindIndexHex: "ab12" }, { emailBlindIndexHex: "ab12" }),
        "email",
      ).comparison,
    ).toBe("agree");
    expect(
      byField(
        compareRecords({ emailBlindIndexHex: "ab12" }, { emailBlindIndexHex: "cd34" }),
        "email",
      ).comparison,
    ).toBe("disagree");
  });
  ...
  test("company falls back from id to registrable domain", () => {
    expect(
      byField(compareRecords({ companyId: "c1" }, { companyId: "c1" }), "company").comparison,
    ).toBe("agree");
    expect(
      byField(
        compareRecords({ companyDomain: "Acme.com" }, { companyDomain: "acme.com" }),
        "company",
      ).comparison,
    ).toBe("agree");
    expect(
      byField(
        compareRecords({ companyDomain: "acme.com" }, { companyDomain: "globex.com" }),
        "company",
      ).comparison,
    ).toBe("disagree");
  });
  ...
  test("end-to-end: strong agreement scores auto_match", () => {
    const a: ComparablePerson = {
      linkedinPublicId: "li-9",
      emailBlindIndexHex: "ff00",
      fullName: "Dana Reed",
    };
    const b: ComparablePerson = {
      linkedinPublicId: "li-9",
      emailBlindIndexHex: "ff00",
      fullName: "Dana Reed",
    };
  ...
  test("end-to-end: a strong-key disagreement scores no_match", () => {
    const a: ComparablePerson = {
      linkedinPublicId: "li-1",
      emailBlindIndexHex: "aaaa",
      fullName: "John Smith",
      companyDomain: "acme.com",
    };
    const b: ComparablePerson = {
      linkedinPublicId: "li-2",
      emailBlindIndexHex: "bbbb",
      fullName: "Xavier Okonkwo",
