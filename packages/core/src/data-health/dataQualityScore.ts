// dataQualityScore.ts — the data-quality & freshness keystone (22 §2–§3, ADR-0025). Pure, set-/row-reusable
// scoring: the 0–100 data_quality_score composite (0.4·completeness + 0.3·verification + 0.3·freshness) with
// the cold-start re-weighting (§2.2), the per-entity completeness weights (§2.3), the verification sub-score
// from email_status/phone_status, and the freshness SLAs + freshness_status bands + continuous decay (§3).
// This is the math everything downstream references — re-verify priority (§4), Data Health (§8), the badge —
// so it lives here, pure and unit-tested; callers (import commit, the freshness sweep, the masked DTO) wire it.
//
...
import type { FreshnessStatus } from "@leadwolf/types";
...
// ── Verification (§2.3) ──────────────────────────────────────────────────────────────────────────────────
/** Verification sub-score for one field status: valid=1 · catch_all/unknown=0.5 · invalid=0. Returns `null`
 *  for an UNVERIFIED/absent status (§2.2 cold start) so it is EXCLUDED from the mean, not penalized as 0. */
export function verificationSubScore(status: string | null | undefined): number | null {
  switch (status) {
    ...
    case "invalid":
      return 0;
    default:
      return null; // unverified / risky-unknown / absent → excluded from the sub-score
  }
}
