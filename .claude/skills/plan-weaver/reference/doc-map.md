...
| 30 | `docs/planning/30-bulk-import-export-pipeline.md` |
| 31 | `docs/planning/31-bulk-enrichment-pipeline.md` |
...
| ADR-36 | `decisions/ADR-0036-bulk-async-job-and-staging-pipeline.md` |
| ADR-39 | `decisions/ADR-0039-bulk-enrichment-pipeline.md` |
...
| 31 | 06, 03, 07, 09, 18, 22, 08, 10, ADR-39, ADR-37, ADR-38, ADR-13, ADR-21, ADR-15, ADR-29 |
...
| ADR-39 | 31, 06, 03, 09, 18 (match-first bulk CSV enrichment pipeline: job/chunk/row model + queue) |
| ADR-37 | 31, 06, 03 (bulk match-first resolution + candidate index; amends ADR-15, extends ADR-21) |
| ADR-38 | 07, 31, 03 (bulk enrichment billing, forecast & quota; extends ADR-13/7/29) |
...
**Bulk enrichment (2026-06-17):** 31↔06/03/07/09/18/22/08/10 (bulk CSV pipeline: match-first +
job/chunk/row model + charge-per-verified-match); 00↔ADR-39/37/38 (decision tripod rows in 00 §7);
...
| **H24** | Bulk enrichment pipeline: `enrichment_jobs`/`_chunks`/`_rows` model + chunked async (`BULK_ENRICHMENT_QUEUE`) + match-first + charge-per-verified-match | 31, 06 §11, 03 (new tables), 07, 09, 18, 10 (M17 + risk #25/#26), ADR-39/37/38 |
| **H25** | Bulk match-first resolution: `MatchPort` resolves internal-first (overlay → master graph → provider) via deterministic keys + blocking/LSH candidate index | 31, 06 §9/§11, 03, ADR-37, ADR-21/15 |
...
| bulk CSV enrichment (upload → match-first → enrich/verify → download; chunked async at scale) | 31 / ADR-39 | 06 §11, 03, 07, 09, 18, 10 (M17), 00 §6/§7 |
| MatchPort / match-first resolution (internal-first: overlay → master graph → provider) | ADR-37 / 31 / 06 §9/§11 | 03, 00 §6/§7, 22, ADR-21/15 |
| `enrichment_job_status` / `match_method` / `match_outcome` enums | 03 (new) / 31 / ADR-39/37 | 06 §11, 09, 07 |
| `enrichment_jobs`/`_chunks`/`_rows` tables (partitioned: `enrichment_job_rows`) | 03 (new) / 31 | 06 §11, 07, 18, ADR-39 |
...
| ADR-21 | Global master graph + per-workspace overlay (two-layer) | Accepted (specialized for bulk by ADR-37) | 02, 03, 06, 08 | 00, 02, 03, 06, 08, 09, 10, 30, 31, README, ADR-37 (reopens ADR-6; revives ADR-3/5; amends ADR-2/15) |
...
| ADR-39 | Bulk CSV Enrichment Pipeline | Accepted | 31, 06, 03, 09, 18 | 00, 06, 03, 07, 09, 10, 18, 22, 08, 31, README |
| ADR-37 | Bulk Match-First Resolution & Candidate Index | Accepted (amends ADR-15, extends ADR-21) | 31, 06, 03 | 00, 06, 22, 31, README |
| ADR-38 | Bulk Enrichment Billing, Forecast & Quota | Accepted (extends ADR-13/7/29) | 07, 31, 03 | 00, 07, 10, 31, README |
