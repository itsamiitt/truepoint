 ...
 *
 * INSTALL (do this when code is first scaffolded):
 *   1. cp this file to the repo root as `.dependency-cruiser.cjs`
 *   2. npm i -D dependency-cruiser     (or: bun add -d dependency-cruiser)
 *   3. add script:  "lint:boundaries": "depcruise apps packages"
 *   4. run it in CI so a forbidden import FAILS the build.
 *
 * Specialized from scalable-architecture/templates/import-boundaries.md (Option C). The Mermaid graph in
 * the navigation map only *visualizes* these edges; THIS file is what *catches* violations.
 */
module.exports = {
  forbidden: [
    {
    ...
    // NOTE: "no process.env outside packages/config" (16 §10) is NOT enforceable here —
    // dependency-cruiser tracks module imports, not property access. Enforce it with an ESLint
    // `no-restricted-properties`/`no-restricted-globals` rule (or review), per ownership-matrix.md.
    {
      name: "no-orphans",
  ...
  options: {
    ...
    // tsConfig: { fileName: "tsconfig.json" },   // uncomment once a root tsconfig exists
