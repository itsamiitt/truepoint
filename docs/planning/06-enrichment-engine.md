...
- **Cost control:** per-provider + global **daily cost budgets** with alerts; the breaker can be tripped
  ...
  **reserve** their worst-case cost against these budgets up front (reserve-then-spend) and **pause/resume** when
  the breaker trips. All spend is observable (cost-per-reveal dashboard, [§10](#10-metrics-the-economics-dashboard)).
...
- **Entity resolution (global — [ADR-0021](./decisions/ADR-0021-global-master-graph-and-overlay.md)).** The
  ...
  **manual review queue**, and `source_records` keeps every merge **reversible** (un-merge). Batch ER runs on
  ...
  [31 §5](./31-bulk-enrichment-pipeline.md)).
...
and the per-stage state machine — lives in [31](./31-bulk-enrichment-pipeline.md); three decisions pin it
down: [ADR-0039](./decisions/ADR-0039-bulk-enrichment-pipeline.md) (pipeline),
...
- **New tables.** `enrichment_jobs` (one per upload), `enrichment_job_chunks` (fan-out unit), and
  ...
  [31](./31-bulk-enrichment-pipeline.md) and [03 §14](./03-database-design.md#14-planned-schema-additions-app-surface--platform).
