...
| MI-1 `/companies/:id` routed page | MI-P1 | ✅ built | `features/companies/` + `GET /api/v1/accounts/:id` (`getMaskedById`, search's SELECTION); reuses prospect sections via barrel; Watch toggle ("Watched accounts" auto-list); drawer + signal rows deep-link. Companies LIST scope (moving Accounts out of the toggle) still owed |
