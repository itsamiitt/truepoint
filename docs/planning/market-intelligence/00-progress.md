...
| MI-1 `/companies` index destination | MI-P1 | ✅ built | `CompaniesIndexPage` reusing the URL-driven account-search engine + filter rail + grid; rail entry (Building2); row → routed page; `contactsHrefForCompany` deep-link builder |
| MI-1 toggle cutover | MI-P1 | ✅ done | Prospect's Contacts⇄Accounts SegmentedControl retired; `?scope=accounts` redirects to `/companies` WITH the shared aq/asort/af query string; contacts surface untouched. Follow-up: `AccountDetailDrawer` is now caller-less — re-wire as a companies-index preview or delete |
| MI-8 Markets board UI | MI-P5 | ✅ built | `/companies/markets` (`MarketsBoard`): per-industry window summary over `/api/v1/market/segments`, honest empty in both dark and no-data states; linked from the Companies index |
...
| Enablement runbook (MI-9) | parallel | ✅ written | [10-enablement-runbook.md](10-enablement-runbook.md) — 8 ordered stages + blocked-stage guardrails + kill order; the flips themselves are deploy-side operator work |
