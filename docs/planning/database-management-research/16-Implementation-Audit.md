...
| **X2** | **Drizzle migration regen** — `validation_rules` (+ earlier `approval_requests`) ship via the `rls/*.sql` **defensive-CREATE** (runtime-safe), but the canonical Drizzle migration + snapshot must be generated. | S1 | **RE-SCOPED 2026-08-22 — "run `drizzle-kit generate` in CI" is NOT a safe instruction as written, and following it would produce a dangerous migration.** Measured on main: **138 journal entries, 41 snapshots, and the contiguous snapshot lineage ends at 0028** — 97 entries from 0029 onward have none (a few strays exist up to 0107, but the chain is broken). `generate` diffs the current schema against the newest lineage it can reconstruct, so run naively it emits ~110 migrations' worth of DDL in ONE file; applied to a database already migrated through 0138 that fails or double-creates. The real choice is a decision nobody has taken: **(a)** rebuild the lineage by introspecting a fully-migrated database into a fresh snapshot and continuing from there, or **(b)** accept the hand-written SQL + `_journal.json` as canonical — which is what the repo has actually done for 110 migrations — and stop treating the snapshot as a source of truth, in which case `validation_rules` needs a hand-written migration like its neighbours rather than a regen. Either way it is a deliberate migration-tooling decision, not a CI chore. **RUNTIME RISK MEASURED AND IS ZERO (2026-08-22):** `applyMigrations` was run end-to-end against a FRESH database — `validation_rules` is created, RLS is **enabled AND forced**, the single `validation_rules_app_read(SELECT)` policy is present, and `leadwolf_app` holds the blanket SELECT/INSERT/UPDATE/DELETE grant that the deliberately-absent write policy neutralises (the `retention_class_policies` posture the file says it mirrors — staff write via `withPlatformTx`, the app can only read). All 138 journal migrations applied. So nothing is broken or exposed while this stays open; what is missing is canonical-migration hygiene, not a working table. The `rls/validationRules.sql` header carried the same unsafe "CI's drizzle-kit generate emits the canonical migration" line and has been corrected in place, since that is where the next person reads it. **Corroborated independently:** the import-and-data-model series already recorded the same fact in passing — its 2026-07-05 S-P5 drift row notes that "snapshots are hand-stopped at 0028 anyway" — so the stop is long-standing and deliberate at the tooling level, and only THIS audit's instruction was written unaware of it. |
