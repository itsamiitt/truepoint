...
| ID | Item | Sev | Status |
|---|---|---|---|
| **X1** | **CI not run** — 8 commits add a new table, RLS, a core engine change (`runImport`), API + UI, all **un-gated** (no `typecheck`/`biome`/itests here). | S0 | **TYPECHECK + BIOME + ITESTS DISCHARGED 2026-08-22** (this audit was written without `bun`; the environment has it now). `turbo typecheck` green for `@leadwolf/db` and `@leadwolf/core`, plus `typecheck:tests` for `@leadwolf/db`. Biome: clean. A repo-wide `bun run lint` IS red on a Windows checkout, but on CRLF only — re-checking the two flagged files (`platformAdminWrites.ts`, `runImport.ts`) as LF copies reports nothing, and the rest of the red is `turbo.json`/`biome.json`/`tsconfig.base.json`-class config files. CI checks out LF, so this half is green there. Itests covering these paths pass: `platformAdminReads` 4, `import` 4, `dataQualitySnapshots` 3, `importPinRespect` 1, `importTemplates` 5. **Still owed:** the drizzle regen (X2 — unchanged) and a CI run on the runner itself; local evidence is not the gate. |
| **X2** | **Drizzle migration regen** — `validation_rules` (+ earlier `approval_requests`) ship via the `rls/*.sql` **defensive-CREATE** (runtime-safe), but the canonical Drizzle migration + snapshot must be generated. | S1 | **RE-SCOPED 2026-08-22 — "run `drizzle-kit generate` in CI" is NOT a safe instruction as written, and following it would produce a dangerous migration.** **CORRECTED 2026-08-22 (an earlier revision of this row overstated the hazard — see the note at the end).** Measured on main: 138 journal entries, 41 snapshots, newest snapshot **0107**. drizzle-kit diffs against the NEWEST snapshot, not a contiguous chain, so a `generate` today emits the delta for the **31** migrations since 0107 — not the whole history. The hazard is real but different: **21 migrations in this tree carry `HAND-AUTHORED (drizzle-kit generate is forbidden)`** because their DDL — RLS policies, partitions, expression/operator-class indexes, the `forge` schema — is not expressible in the schema files drizzle diffs, so a regen emits spurious or destructive statements for exactly those. That is the settled house convention, not an open question: hand-authored SQL is canonical. **RESOLVED 2026-08-22 (commit 5958603b):** `migrations/0139_validation_rules.sql` is that hand-written migration, and `rls/validationRules.sql` was trimmed to the policy + grant it should own. Verified on real databases — a fresh one yields identical structure (8 columns, both indexes, RLS enabled AND forced, the SELECT policy, the same grants) at 139 migrations; `applyMigrations` run twice is a no-op, which is the production path since every existing database already carries the table from the defensive CREATE; and the journal's two pre-existing integrity breaks sit at the same positions before and after, so the append introduced neither. Either way it is a deliberate migration-tooling decision, not a CI chore. **RUNTIME RISK MEASURED AND IS ZERO (2026-08-22):** `applyMigrations` was run end-to-end against a FRESH database — `validation_rules` is created, RLS is **enabled AND forced**, the single `validation_rules_app_read(SELECT)` policy is present, and `leadwolf_app` holds the blanket SELECT/INSERT/UPDATE/DELETE grant that the deliberately-absent write policy neutralises (the `retention_class_policies` posture the file says it mirrors — staff write via `withPlatformTx`, the app can only read). All 138 journal migrations applied. So nothing is broken or exposed while this stays open; what is missing is canonical-migration hygiene, not a working table. The `rls/validationRules.sql` header carried the same unsafe "CI's drizzle-kit generate emits the canonical migration" line and has been corrected in place, since that is where the next person reads it. **The authoritative account is `0083_tan_wolfpack.sql`** — an intentionally EMPTY migration that exists solely to carry a correct snapshot, rebaselining the chain after it broke at 0029. Its header records what a blind regen did at the time (722 lines of `CREATE TABLE` for tables that had existed for dozens of migrations, silently) and why it would not even fail loudly (`applyMigrations` tolerates "already exists" by design). **Correction to the previous revision of this row:** it claimed a regen would emit ~110 migrations of DDL and that the choice was unmade. Both were wrong — 0083 already rebaselined, so the diff base is 0107, and the hand-authored convention is already established in 21 migration headers. The earlier framing came from measuring snapshot CONTIGUITY, which drizzle-kit never consults. |
...
| **X4** | **`main` promotion** — branch was 8 commits ahead; promotion is the user's action. | — | **RESOLVED 2026-08-22 — already merged, this row was stale.** `git rev-list origin/main..origin/feat/data-mgmt-01-research-brief` = **0**: main contains every commit the branch has (and is 149 ahead). The schema, repositories and routes are all present in the main tree. Nothing is waiting to be promoted; a promotion today would be a no-op. |
...
> **Register re-checked 2026-08-22:** every remaining gap is blocked BY DESIGN — A1/A2/A3 NEEDS-REVIEW,
> A4/A9/A10/A11 DEFERRED, A1/A7/A8 CRED-GATED, A6 NEEDS-DATA, A5 NOT-BUILT (large). There is no
> safely-buildable item left in this table, so a future session should not re-scan it hoping for one: the
> next move is a human decision (X3 security review, credentials, sign-off), not more code. The
> cross-cutting rows are now clear — X1 gates run, X2 migration shipped, X4 already merged.
