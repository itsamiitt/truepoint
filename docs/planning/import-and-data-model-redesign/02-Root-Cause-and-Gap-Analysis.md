...
| **G21** | — | No duplicate-*resolution* surface: `duplicate_of_contact_id` markers (and I5 `match_links`) accumulate with only a dismiss/unmark-only list shipped (`data-health` DuplicatesSection, 01 §6.9) — no queue/UI to accept or merge | P2 | RC-5 | §6.9 | §2.1, §2.3 [8][34][88] (persistent review queues) | `11` (in-place upgrade; + `04` semantics) |
