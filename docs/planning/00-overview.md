...
- **Workspace isolation is absolute.** Contacts/accounts are per-workspace; RLS enforces it in the DB.
...
- **AWS-native, self-hosted** ([ADR-0010](./decisions/ADR-0010-aws-native-self-hosted-stack.md)); design
  for 100M+ from day one, ship the simplest correct thing first.
...
| **Contact / Account** | A person / company **copy owned by one workspace** (not shared across workspaces). |
...
| Search | **Typesense self-hosted, day one** (behind SearchPort) | Scale + AWS-native → [ADR-0002](./decisions/ADR-0002-search-postgres-then-engine.md) |
...
| Entity resolution | **Splink probabilistic dedup** (within-workspace; Zingg alt) | Dedup quality, risk #1 → [ADR-0015](./decisions/ADR-0015-entity-resolution-dedup-engine.md), [06 §9](./06-enrichment-engine.md) |
...
| Scale | 100M+ → partitioning + Typesense + ClickHouse | → [03](./03-database-design.md), [01](./01-tech-stack.md) |
...
3. **Per-workspace dedup quality** without a global identity — match-on-import heuristics ([03 §13](./03-database-design.md)).
