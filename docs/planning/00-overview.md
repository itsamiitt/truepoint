...
| **Bulk CSV enrichment** | Upload a sparse CSV → match each row against our own data (master graph + overlay) → enrich/verify → download, run as a chunked async job at enterprise scale ([31](./31-bulk-enrichment-pipeline.md), [ADR-0039](./decisions/ADR-0039-bulk-enrichment-pipeline.md)). |
...
| Entity resolution | **Global/cross-source: deterministic keys + blocking/LSH + Splink + survivorship** | Dedup the universe, risk #1 → [ADR-0015](./decisions/ADR-0015-entity-resolution-dedup-engine.md) (amended by [ADR-0021](./decisions/ADR-0021-global-master-graph-and-overlay.md), [ADR-0037](./decisions/ADR-0037-bulk-match-first-resolution-and-candidate-index.md)), [06 §9](./06-enrichment-engine.md) |
...
| Search UX | Advanced faceted filters (incl. **intent**), **search-box typeahead from indexed values**, **abbreviation/synonym expansion** (CEO→Chief Executive Officer), saved views, smart segments, instant masked search | Apollo/ZoomInfo-grade exploration → [24](./24-advanced-search-exploration-ux.md), [ADR-0035](./decisions/ADR-0035-search-query-and-filter-architecture.md) |
...
| Bulk import/export | **First-class async bulk job** — presigned/multipart-S3 upload → AV scan → streaming parse → COPY-to-staging → `ON CONFLICT` upsert; idempotent, checkpointed, resumable; three-way per-row accounting (success/failed/unprocessed) + downloadable rejected-rows report; snapshot-consistent streaming export (gzip/shard/manifest) | Million-row CSV import/export at Salesforce-Bulk-API/HubSpot parity → [30](./30-bulk-import-export-pipeline.md), [ADR-0036](./decisions/ADR-0036-bulk-async-job-and-staging-pipeline.md) |
...
| Bulk enrichment | Match-first bulk CSV pipeline (chunked async, internal-match-first, provider residual) | → [ADR-0039](./decisions/ADR-0039-bulk-enrichment-pipeline.md), [31](./31-bulk-enrichment-pipeline.md) |
