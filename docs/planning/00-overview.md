...
| Bulk match-first | **Bulk CSV rows resolve via a `MatchPort`: overlay deterministic → master-graph candidate index (free, infra-gated stub) → provider residual**; internal matches free + instant | Bulk in a fraction of a minute; providers billed only on residual → [ADR-0037](./decisions/ADR-0037-bulk-match-first-resolution-and-candidate-index.md) (amends ADR-0015, extends [ADR-0021](./decisions/ADR-0021-global-master-graph-and-overlay.md)), [30](./30-bulk-csv-enrichment.md), M17 |
