...
|---|---|---|---|
| **P0** *(remaining — the last optimization)* | Non-blocking lookup: `/lookup`, on stale/absent, `registerUrl` + return instantly with `freshness`/`refresh` instead of awaiting `fetchAndLandUrl`. Now safe to land — the consumer exists (P2), so the async result reaches the card via the push. | `contacts-resolve/routes.ts`, `@leadwolf/types` | S-03 (instant card) |
...
| **P2** ✅ *loop closed (dark)* | Real-time subject-update topic, end-to-end via §13 option D. **Shipped:** panel live-refresh (CapturedTab @a9fbf9b4); event `EVENT_CONTACT_LOOKUP_UPDATED` reserved (@b0d02d7f); producer — sweep emits to interested workspaces post-landing (@bfa100db); consumer — `eventStream` → `SUBJECT_STATUS` (@6807100b); `/lookup` registers interest (@4f8f2234). All dark behind `REALTIME_SSE_ENABLED`. Left: RevealTab hydrate-on-open (minor); an itest with the gate on. | `realtimeEvents.ts`✅, `lookupInterest.ts`✅, `linkedinLinkFetchSweep.ts`✅, `resolver.ts`✅, `eventStream.ts`✅, `contacts-resolve/routes.ts`✅ | S-10/S-13 (live update) |
