...
| **P2** *(in progress)* | Real-time subject-update topic. **Done:** panel live-refresh on drain (CapturedTab, @a9fbf9b4); event contract reserved — `EVENT_CONTACT_LOOKUP_UPDATED` + payload (@ this commit), dark. **Remaining:** outbox producer in the landing tx (routes the requesting workspace's channel), wire the dark SSE client's payload → `SUBJECT_STATUS`, RevealTab hydrate-on-open | `realtimeEvents.ts`✅, `landSourcePayload.ts`, `eventStream.ts`, `Panel.tsx`(partial) | S-10/S-13 (live update) |
