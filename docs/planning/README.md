...
| 32 | [Database Audit → Frontend & API Plan](./32-database-audit-frontend-api-plan.md) | Post-Layer-0 full audit of the as-built schema (152 tables), API (~200 endpoints), repositories and both frontends; the consolidated issue register (P0–P2) and the waved plan to surface the intelligence layer [S-10][S-13]. |
| 33 | [Frontend Plan for the Graph Model](./33-frontend-plan-for-the-graph-model.md) | What `apps/web` should build after migration 0108 — sequenced by which Layer-0 tables actually have producers. Zero new routes (consolidation ruling); provenance/confidence first because it is the only new data that exists; technology/education/alumni deliberately deferred. |
