...
| 1.3-ui | Admin console **auth module** (policy config UI in the existing `admin.truepoint.in` shell) — view platform defaults (1.3-read) + set a key (1.2c-staff PUT) + view a tenant's effective policy | M | ◻ next | **Shell + both API endpoints now exist** — needs the Next page(s) + form + the DS components, matching an existing admin section. Frontend (local-validation only — unit gate blocked in CI). |
