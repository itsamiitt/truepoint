...
| **WebAuthn / passkeys** — enrollment UI + sign-in integration | AUTH-024 | M | ◻ next | The account-security "Add a passkey" UI (`@simplewebauthn/browser` `startRegistration` → the routes above) + the authentication routes + the passkey sign-in flow. App-layer; the register backend + routes are ready. |
