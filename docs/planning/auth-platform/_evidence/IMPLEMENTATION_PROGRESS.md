...
| **WebAuthn / passkeys** — registration + assertion CEREMONY | AUTH-024 | XL | ◻ **flag for specialist review** | The security-critical part: a WebAuthn library (dep), RP-ID for the subdomain estate, attestation policy, anti-replay/counter checks, the challenge store, routes + UI. Build off-by-default + gated; **needs specialist review before enable**. The schema above is the proven foundation. |
