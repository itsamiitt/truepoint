...
| @leadwolf/auth-client **extract** (DRY web+admin clients) + admin callback client-nav; KMS-managed at-rest key + versioned re-encrypt | AUTH-073/013 | M | ◻ | Remaining Phase-2 items. The extract would collapse the now-twice-duplicated single-flight/refresh logic; the KMS item is infra-dependent (needs a real KMS for the envelope). |
