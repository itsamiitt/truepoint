...
| 1.2c-endpoint | The policy **API/admin endpoint** (authn + staff-RBAC → resolve floor → `validatePolicyWrite` → `upsertTenantKey`/`setPlatformKey`; delete-on-write cache) | M (AUTH-021) | ◻ next | App-layer. All DB primitives + the pure `validatePolicyWrite` decision are done, so the endpoint is thin wiring. Read truepoint-platform (API) + truepoint-security (staff-RBAC) first. |
