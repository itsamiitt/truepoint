...
| **WebAuthn / passkeys** — registration + assertion CEREMONY | AUTH-024 | XL | ◻ **flag for specialist review** | The security-critical part: a WebAuthn library (new dep), generate/verify registration + authentication (RP-ID for the subdomain estate, origin check, attestation policy, counter/clone detection binding to the challenge store above), the repository writes, routes + UI. Build off-by-default + gated; **needs specialist review before enable**. Foundation ready: credential schema (CI-proven) + challenge store. |
