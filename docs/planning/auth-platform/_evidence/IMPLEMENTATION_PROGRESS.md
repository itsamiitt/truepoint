...
| 1.1b-switch | Switch `flow.ts` `finalizeLogin` (+ refresh gates) to read the RESOLVED policy instead of `tenant_auth_policies` directly, + backfill `tenant_auth_policies` → `auth_policies` | part of L | ⏸ **deferred (behaviour change, needs DB)** | Live login-enforcement change + a DATA migration — too risky to flip/validate blind. Do with a DB after the 1.1b-wire itest is green. |
...
| 1.2c | **Platform** write `setPlatformKey` (withPlatformTx, staff-only, platform_audit_log; NULL-tenant onConflict) + the **API/admin endpoint** (authn+staff-RBAC → resolve floor → `validatePolicyWrite` → upsert; delete-on-write cache) | M (AUTH-021) | ◻ next | Blind/app. The pure decision (1.2c-decide) + tenant write (1.2b) are done, so the endpoint is thin. Read truepoint-platform (API) + truepoint-security (staff-RBAC) first. |
...
| 1.4b | Allowed-origins **DB + wiring**: `auth_allowed_origins` table `(scope, tenant_id?, origin, kind)` + RLS + isolation itest; repository; `isAllowedOrigin` call sites read env floor ∪ managed | M (AUTH-036) | ◻ next | Blind/DB (same pattern as 1.1a). The write path stores `canonicalManagedOrigin(input)`; the redirect/CORS guards resolve floor ∪ managed. |
...
| 1.5b | Observability **wiring**: `recordAuthMetric` call sites (login success/fail in flow.ts, token mint in exchange, revocation check in authn/revocation, policy-block in the gates, mfa challenge), a `/metrics` scrape endpoint on the auth+api surfaces, and the SLI dashboard/alert spec | M | ◻ next | Increment call sites across the flows + expose `renderAuthMetrics()`. Emission is verifiable per-site; dashboards/alerts need the infra. |
