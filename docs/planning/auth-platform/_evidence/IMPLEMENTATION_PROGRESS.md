...
| 1.5b-rest | Remaining: login **failure** metric (`login.ts`), the OTHER enforcement gates (`ip`/`sso`/`session`/`idle`), `auth_mfa_challenge_total` (MFA path) | S | ◻ next | The scrape endpoints + the highest-value SLIs (login-success, method-block, token-mint, revocation) are all live; these are the completeness tail. |
