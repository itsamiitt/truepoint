...
| Item | AUTH | Effort | Status | Notes |
|---|---|---|---|---|
| **Email OTP** MFA factor — core + challenge backend | AUTH-025 | M | ✅ **backend done** | Core: `verifyMfaCode` routes `"email_otp"` (shared auth_email_tokens store, VERIFIED email, single-use+TTL) + `requestEmailOtp(userId)`. Challenge backend: `submitMfa` is now method-aware (form `method`, default `"totp"` = today's exact behaviour, `email_otp` accepted, anything else rejected) + `checkEmailOtpSendRate(userId)` anti-mailbomb throttle (3 sends / 15 min, own keyspace, fail-open). All additive + backward-compatible — INERT until the challenge UI offers email-OTP. `mfaVerify.test.ts` 5/5; packages/auth 138/138. typecheck ✓ biome ✓. **Follow-up (app layer, login-critical UI): the mailer-send action wiring + the MFA challenge/enrollment UI — best done with review.** |
