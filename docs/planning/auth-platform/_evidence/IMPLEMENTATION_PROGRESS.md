...
| 1.1b-core | Effective-policy **resolver — pure core**: `composeEffectivePolicy(platformDefault, …overrides)` (N-scope strictest-wins fold) + `assembleScopePolicy(rows)` (auth_policies rows → typed `Partial<AuthPolicy>`) | part of L | ✅ **done (fully verified, no DB)** | `packages/auth/src/policy.ts` — generalizes the proven 2-scope `resolveEffectivePolicy` to platform→org→workspace by folding pairwise (per-key ops are associative + monotonic, so a child can only TIGHTEN). `assembleScopePolicy` reuses `authPolicySchema.shape.*` validators (single source of truth) and SKIPS a malformed row rather than throwing (a bad row must not break login). +11 tests (`policy.test.ts` — also backfills coverage for the previously-untested resolveEffectivePolicy/strictestMfa/isMethodAllowed). typecheck ✓ biome ✓ tests 11/11 ✓. |
| 1.1b-wire | Effective-policy resolver **wiring**: `effectivePolicyRepository` (reads `auth_policies` per scope via withTenantTx — own + platform-NULL rows), Redis cache keyed `(tenant, workspace, version)` w/ write-invalidation, + backfill `tenant_auth_policies` → `auth_policies` | part of L | ◻ next | Blind (DB/Redis) — repo read + cache + backfill migration; resolver-correctness itest through withTenantTx. Switch `flow.ts` finalizeLogin to read the resolved policy from here (currently reads tenant_auth_policies directly). |
