...
| 1.3-ui-edit | The **edit** flow — a `Dialog` to set a platform-default key (`PUT /admin/auth/platform-policy`, already floor-guarded server-side), with a per-key value input (mfa enum → select, booleans → switch, timeouts → number, arrays → textarea) + `useToast` + reload | M | ◻ next | The `EditPolicyDialog` retention analogue; the write endpoint + validation already exist, so this is the adaptive value form + wiring. |
