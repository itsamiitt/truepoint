...
| 1.5b-rest | Remaining: login **failure** metric (`login.ts`), the OTHER enforcement gates (`ip`/`sso`/`session`/`idle`), `auth_mfa_challenge_total` (MFA path), and the **apps/auth `/metrics` route** (Next handler — exposes login/token SLIs; apps/auth is a distinct process from apps/api) | M | ◻ next | Mirror the apps/api endpoint on the Next auth app (same METRICS_TOKEN gate) so the login/token counters wired in flow.ts/token.ts become scrapeable. |
