...
| **WebAuthn / passkeys** — AUTHENTICATION ceremony + routes + UI | AUTH-024 | L | ◻ next | The assertion pair (generate/verify, counter-advance clone check) mirroring registration, + the gated `/account/security/passkeys/*` routes and the enroll/sign-in UI (`navigator.credentials.*`). App-layer; gated on WEBAUTHN_ENABLED. |
