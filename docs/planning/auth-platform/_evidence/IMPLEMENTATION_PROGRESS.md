...
| 1.2c | **Platform** write path `setPlatformKey` (withPlatformTx, staff-only, platform_audit_log) + the app-layer **write orchestration** (parse value → fetch floor → findFloorViolations → upsert; delete-on-write cache) + the API/admin endpoint | M (AUTH-021) | ◻ next | Platform upsert (NULL-tenant onConflict) via the owner tx; the orchestration ties the two pure guards + the DB write; then the `admin.truepoint.in` endpoint. |
