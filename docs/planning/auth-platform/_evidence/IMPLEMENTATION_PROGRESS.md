...
| 1.1b-wire | Effective-policy resolver **DB wiring**: `packages/db` read `getScopeRows({tenantId, workspaceId})` (bare SELECT under withTenantTx → RLS returns platform-NULL + own-tenant rows) feeding `resolvePolicyFromRows`; Redis cache keyed `(tenant, workspace, version)` w/ write-invalidation; resolver-correctness **itest** | part of L | ◻ next | Blind (DB/Redis), itest-gated. ARCH: `packages/db` can't import `@leadwolf/auth` (wrong dep direction), so DB returns raw rows + `@leadwolf/auth` composes via `resolvePolicyFromRows`. |
| 1.1b-switch | Switch `flow.ts` `finalizeLogin` (+ refresh gates) to read the RESOLVED policy instead of `tenant_auth_policies` directly, + backfill `tenant_auth_policies` → `auth_policies` | part of L | ⏸ **deferred (behaviour change, needs DB)** | This changes LIVE login enforcement + is a DATA migration — too risky to flip/validate blind. Do with a DB after 1.1b-wire's itest is green. |
