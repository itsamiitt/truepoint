...
| 1.2d | **Resolve-time floor (defense-in-depth) — security-design decision for review** | S | ◻ **flagged for review** | Found reviewing the resolver: the floor is a WRITE-time guard only. `resolvePolicyFromRows` composes `{...floor, ...platformRows}` (platform OVERRIDES the floor) and TRUSTS `validatePolicyWrite`, so it is NOT re-clamped at resolve time — a sub-floor platform row (from a **floor RAISE** after the row was written, a direct DB write, or a migration) would resolve BELOW the raised floor. Two principled fixes, a design call for the reviewer: **(a)** clamp the composed policy up to the floor as the last resolve step (`resolveEffectivePolicy(floor, composed)` — a no-op unless a row is sub-floor, so zero normal-case change) — silent defense-in-depth; or **(b)** keep write-only enforcement and build the `findFloorViolations` **consumer** (its tested-but-unwired primitive, AUTH-021): a platform-admin **floor-raise remediation** report listing which existing policies violate a newly-raised floor, for explicit fixing. Flagged, not changed unilaterally — it touches the core security resolver. |
...
| 1.5b-gates | The enforcement-gate block reasons: `auth_policy_block_total{reason:"ip"}` at the IP-allowlist gate (flow.ts) + `{reason:"idle"}` at the idle-timeout gate (refresh.ts) | XS | ✅ **done** | Additive, at the two clean gate sites. `sso` folds into the already-wired `method` reason (isMethodAllowed enforces requireSso); `session` (absolute cap) is enforced inside the session lookup's expiry/reuse check, not a distinct site — left unmetered. **All login-path + gate block SLIs now wired.** typecheck ✓ biome ✓ packages/auth 133/133. |
...
| `__Host-` refresh cookie — **dual-READ (readers-first)** | AUTH-074 | S | ✅ **done (read stage)** | `cookies.ts` adds `REFRESH_COOKIE_HOST="__Host-lw_refresh"` + `readRefreshToken(jar)` / `readRefreshTokenFromHeader(header)` (host-preferred, legacy fallback); all 4 read sites (requireUser, sessionGuard, token/refresh, logout) now dual-read. INERT + safe today — nothing writes the new name yet, so it always falls back to the legacy cookie (behaviour unchanged). Staged readers-first so every instance can READ the new cookie before any WRITES it. `cookies.test.ts` 5/5. typecheck ✓ biome ✓. |
| `__Host-` refresh cookie — **WRITE flip + clear-both** | AUTH-074 | S | ✅ **done (env-gated)** | All setters (`refreshCookie` string form → org/switch, workspace/switch, token/refresh; `finishLogin` jar.set) now emit the name/attributes from `refreshCookieName()`/`refreshCookieWritesHost()`, gated by `REFRESH_COOKIE_HOST_WRITE` (default OFF = legacy `lw_refresh`+Domain = today). Under the flag: `__Host-lw_refresh`, NO Domain. `clearRefreshCookie()` now returns BOTH clears (host no-Domain + legacy with-Domain); all 4 clear sites append each. Pure builders `buildRefreshSetCookie`/`buildClearRefreshCookies` unit-tested (8/8 in cookies.test.ts). **Rollout: deploy (flag OFF, reads accept both) → verify → flip `REFRESH_COOKIE_HOST_WRITE=true` (env only, no redeploy).** typecheck (auth/config) ✓ biome ✓. |
| Admin refresh **single-flight** | AUTH-078 | S | ✅ **done** | apps/web already single-flighted `silentRefresh`; apps/admin did NOT — so on cold load the adminGate + first data fetch each POSTed /token/refresh, each rotated the refresh token, and the 2nd presented an already-rotated token → the auth service's reuse-detection revoked the whole session family (a spurious staff logout). Added the same `refreshInFlight` collapse (mirrors web). typecheck (apps/admin) ✓ biome ✓. |
...
| Item | AUTH | Effort | Status | Notes |
|---|---|---|---|---|
| **Email OTP** MFA factor | AUTH-025 | M | ✅ **done (functional end-to-end)** | Core: `verifyMfaCode` routes `"email_otp"` (shared auth_email_tokens store, VERIFIED email, single-use+TTL) + `requestEmailOtp`. Backend: `submitMfa` method-aware (default `"totp"` = today's exact behaviour) + `checkEmailOtpSendRate` anti-mailbomb throttle (3/15min, fail-open). UI: the `/mfa` page keeps TOTP as the DEFAULT and adds an "Email me a code instead" option → `sendEmailOtp` action (rate-limit → `requestEmailOtp` → `sendAuthEmail` with the new **login-code** email template, best-effort/never-leak) → `?method=email_otp` variant of the same code form. All ADDITIVE — the TOTP path is byte-for-byte unchanged. `mfaVerify.test.ts` 5/5; typecheck (packages/auth + apps/auth) ✓ biome ✓. Enrollment (email-OTP is auto-available since the email is already verified — no separate enroll) — a per-tenant "allow email-OTP fallback" toggle is a possible policy-key follow-up. |
| **WebAuthn / passkeys** — schema foundation | AUTH-024 | M | ✅ **foundation authored → CI-validating** | `webauthn_credentials` table (user-owned like user_mfa_methods; credential_id UNIQUE, COSE public_key bytea, signature counter, transports, backed_up, label) + migration 0055 + journal. Defense-in-depth: **REVOKED from leadwolf_app** (the tenant app never touches passkeys — can't even enumerate credentials; noted the older user-scoped auth tables still ride the blanket grant as a follow-up). `webauthnCredentialsIsolation.itest.ts`: owner can store; leadwolf_app denied read+write; credential_id unique. typecheck ✓ biome ✓; pushed → CI (Postgres). |
| **WebAuthn / passkeys** — challenge store | AUTH-024 | S | ✅ **done (foundation)** | `webauthnChallenge.ts`: `storeWebauthnChallenge` / `consumeWebauthnChallenge` (Redis, 5-min TTL, one in-flight per user+ceremony). SINGLE-USE via atomic `GETDEL` — a captured challenge can't be replayed (the anti-replay binding at the heart of WebAuthn). Library-independent + inert until the ceremony wires it. typecheck ✓ biome ✓ packages/auth 138/138. |
| **WebAuthn / passkeys** — credential repository | AUTH-024 | S | ✅ **authored → CI-validating** | `webauthnCredentialRepository` (owner-connection CRUD: listForUser / findByCredentialId / create / updateCounter — the table is REVOKEd from leadwolf_app). `webauthnCredentialRepo.itest.ts`: create → list/find round-trips public key+counter+transports → updateCounter → unique credential_id. typecheck ✓ biome ✓; pushed → CI. |
| **WebAuthn / passkeys** — REGISTRATION ceremony ⚠ **review before enable** | AUTH-024 | L | ✅ **authored (off by default)** | Added the vetted `@simplewebauthn/server@13` (NOT hand-rolled crypto). `webauthnRegistration.ts`: `generatePasskeyRegistration` (excludeCredentials + stash single-use challenge) + `verifyPasskeyRegistration` (verify attestation vs the stored challenge + `expectedOrigin`=APP_ORIGINS + `expectedRPID`=`WEBAUTHN_RP_ID`, then persist). Config `WEBAUTHN_ENABLED` (off) + `WEBAUTHN_RP_ID`. **⚠ NEEDS SPECIALIST REVIEW** (RP-ID/origin/attestation/challenge-binding) before the routes enable it. typecheck ✓ (validates the v13 API) biome ✓ packages/auth 138/138. |
| **WebAuthn / passkeys** — AUTHENTICATION ceremony ⚠ **review before enable** | AUTH-024 | M | ✅ **authored (off by default)** | `webauthnAuthentication.ts`: `generatePasskeyAuthentication` (allowCredentials = the user's creds + single-use challenge) + `verifyPasskeyAuthentication` (consume challenge → the presented credential MUST belong to the acting user → verify assertion vs origin/RP → the lib enforces the monotonic counter → persist advanced counter). User-scoped (usernameless/discoverable variant is a follow-up). typecheck ✓ (v13 API) biome ✓ packages/auth 138/138. Both ceremonies now built. |
| **WebAuthn / passkeys** — enrollment ROUTES | AUTH-024 | S | ✅ **done (gated)** | `POST /account/security/passkeys/register/options` + `.../verify` route handlers — gated on `WEBAUTHN_ENABLED` (404 when off) + a live session (401 via the new `resolveApiUser`, a non-redirecting `requireUser` variant for JSON endpoints; requireUser refactored onto it). The verify route fails closed. Fixed a `Uint8Array<ArrayBuffer>` nuance the cross-package (apps/auth DOM-lib) typecheck caught. typecheck (apps/auth + packages/auth) ✓ biome ✓ packages/auth 138/138. |
| **WebAuthn / passkeys** — account management UI (add/list/remove) | AUTH-024 | S | ✅ **done (gated)** | `PasskeySection.tsx` (added `@simplewebauthn/browser@13`): lists the user's passkeys (label/synced/last-used, no public key) + "Add a passkey" (`startRegistration` → options/verify routes; cancelled prompt = soft notice) + per-passkey Remove. Routes: `GET /passkeys` (summary), `DELETE /passkeys/[id]` (ownership-checked, 204/404), + the register options/verify. All gated on `WEBAUTHN_ENABLED` (404 when off) + a live session (401) → fully inert until enabled. Repo: `listSummaryForUser` + `deleteForUser` (matches userId AND id). typecheck (db + apps/auth) ✓ biome ✓. **Passkey enrollment + account management complete end-to-end** (schema → store → repo → ceremonies → routes → UI). |
| **WebAuthn / passkeys** — hardening (audit + step-up + metric) | AUTH-024 | S | ✅ **done** | Reviewer's-eye pass over what's built: (a) `webauthn_ceremony_total{ceremony,result}` SLI on both ceremonies; (b) `auth_mfa_challenge_total` now counts the passkey second factor; (c) **audit** passkey add/remove (`auditPasskeyChange` dual-sink → audit_log / platform_audit_log; both closed-enum coverage tests updated); (d) **step-up** — add/remove now re-prove the current password/TOTP (`verifyStepUp`, 403 otherwise) + a "Confirm it's you" input in the UI, closing the gap vs MFA enroll (stepUp.ts: "every state-changing account action"). Boot guard: `WEBAUTHN_ENABLED` requires `WEBAUTHN_RP_ID`. Ceremony orchestration unit-tested (10). |
| **WebAuthn / passkeys** — sign-in (MFA factor) ⚠ **review before enable** | AUTH-024 | M | ✅ **authored (off by default)** | Passkey as a SECOND factor at /mfa (additive; TOTP/email-OTP paths byte-for-byte unchanged): `POST /mfa/passkey/options` (txn-scoped challenge) + `submitMfaPasskey` server action (mirrors submitMfa EXACTLY — same mfa: lockout namespace, verifyPasskeyAuthentication, mark-verified, resolveNextStep→finishLogin) + `PasskeySignIn` "Use a passkey" button (`startAuthentication`). Gated on WEBAUTHN_ENABLED (the option only renders + the route 404s when off). typecheck (apps/auth + packages/auth) ✓ biome ✓ packages/auth 138/138. **⚠ touches the login flow — review before enable.** **The XL passkey feature (AUTH-024) is now built END-TO-END** (schema → ceremonies → enrollment → management → sign-in), all off-by-default + review-flagged. Follow-ups: usernameless/first-factor passkey login (backend now built — see next row); passkey as a login METHOD. |
| **WebAuthn / passkeys** — usernameless (passwordless) login | AUTH-024 | M | ◧ **backend done; wiring flagged** | BACKEND (safe, decision-independent — built + 5 unit tests): `generatePasskeyAuthenticationUsernameless` (no allowCredentials + `userVerification: "required"`; challenge keyed by a random single-use `ul_` handle, no userId yet) + `verifyPasskeyAuthenticationUsernameless` (verify the assertion → **resolve the user FROM the credential** → advance counter → return userId; no cross-user check needed — the owner IS the user). REMAINING (flagged, review-gated): the WEBAUTHN_ENABLED-gated `/login/passkey/*` routes + the "Sign in with a passkey" login-page button + the finalizeLogin-from-passkey path — and the **passkey-as-MFA decision**: does a user-verified passkey first factor SATISFY an org's `mfa_enforcement=required` (skip the separate MFA step, `mfaVerified=true`), or is the conservative `mfaVerified=false` (passkey then still MFAs, safe but redundant UX) the default? A security-policy interaction — flagged. |
| **Auth hardening sweep** — audit / notify / step-up completeness | AUTH-024/067 | S | ✅ **done** | Reviewer's-eye pass beyond passkeys — each a gap measured against the codebase's own standards for MFA/password: (a) **MFA-outcome audit** — `submitMfa` + `submitMfaPasskey` now `recordPlatformAuthEvent` `mfa.success`/`mfa.failure` `{method}` (were declared-PENDING); (b) **`session.revoked`** fully wired via the `password.reset` dual-sink — the tenant-less/multi-tenant branch was UNAUDITED (PENDING→WRITTEN in both coverage tests); (c) **passkey-changed security email** (`notifyPasskeyChange`, parity with `notifyMfaChanged`); (d) passkey ownership-check + no-key-summary **itests** (CI-validated on Postgres); (e) passkey add/remove **step-up** re-auth (was bypassing `verifyStepUp`). Verified-solid (no change needed): OTP uses a CSPRNG, `/metrics` is `METRICS_TOKEN`-gated, the MFA SLI covers both factors, `sessionsToEvict` is tested, the mailer emits alertable MISCONFIGURED markers (no silent-success), magic-link is wired. |
| **Trusted devices** ("remember this device 30d" MFA skip) | device.trusted | M | ◻ **flagged — MFA-BYPASS surface, review before build** | Found half-scaffolded during the sweep: the `trusted_devices` TABLE exists, the `/mfa` checkbox exists (now HIDDEN behind `TRUSTED_DEVICES_ENABLED`, off — it was a no-op **trust bug**: users "remembered" a device and MFA re-prompted anyway), `newSignInEmail` is a **DEAD template** (new-device notification unwired), and `device.trusted`/`device.revoked` are declared-PENDING audit actions. To build: a device-token store (hashed, per-user, TTL), the MFA-skip check, revocation, and wire `newSignInEmail`. Because it **skips MFA**, build it off-by-default + specialist review (like passkeys) — not a rushed autonomous build. |
...
| Item | AUTH | Effort | Status | Notes |
|---|---|---|---|---|
| **SSO no-lockout guard** ("ship first") | AUTH-031 | S | ✅ **done** | Found the SSO seam already exists (tenant_sso_configs + ssoConfigRepository, `sso/jit.ts` JIT, SCIM, ssoRoutes, `sso/providers.ts` with a mock IdP + throwing-stub real adapters). The unbuilt "ship-first" gap was the no-lockout guard: `providers.ts` `isSsoProviderWired` (non-prod = mock/wired; prod gated on `WIRED_PROD_PROTOCOLS`, empty until arctic/node-saml land — the single flip-point) + pure `ssoReadyForEnforcement(config)`; the settings route rejects `require_sso=true` with **403 `sso_not_ready`** unless the org's connection is enabled AND its provider wired (else forcing SSO with a throwing adapter locks everyone out). BOTH write scopes covered: the **platform-default** write (admin PUT /auth/platform-policy) rejects `require_sso=true` entirely (**403 `require_sso_not_platform_default`**) — a platform-wide force-SSO can't be verified per-tenant and would mass-lock-out every tenant without SSO. Build-vs-buy-INDEPENDENT. providers.test 4/4; typecheck (auth+api) ✓ biome ✓. |
| **SSO enforcement: `enforced` vs `require_sso`** — reconcile | — | S | ◻ **flagged for review** | Found during the pass: TWO independent "force SSO" representations. `tenant_sso_configs.enforced` is set by the SSO config-panel checkbox, stored, and read by `loadSsoConfig` — but it is **not enforced at login**; `ssoConfigRepository.upsert` writes it WITHOUT touching `require_sso`, and the login gates enforce `require_sso` (the effective-policy engine key, via `isMethodAllowed`), not `config.enforced`. So ticking "enforce SSO" in the panel does NOT force SSO unless `require_sso` is also set (now guarded by the no-lockout check). Likely `enforced` is the pre-engine mechanism superseded by `require_sso` (Phase-1 shadow→cutover). Decide: deprecate `enforced` (drive everything off `require_sso`), or have the panel write `require_sso` through the guarded policy write. An SSO-enforcement design call — flagged, not changed unilaterally. |
| **Real SAML/OIDC adapters** | AUTH-001/… | XL | ◻ **flagged — build-vs-buy (research)** | The `oidcProvider`/`samlProvider` bodies in `providers.ts` throw until wired; the mock exercises the whole flow. Buy (WorkOS/Scalekit) or build (`arctic` OIDC + `@node-saml` SAML — assertion validation is the highest-risk surface: anti-XXE / anti-signature-wrapping / reject-unsigned). Same `SsoProvider` interface + the seam above either way — see the [SSO integration decision brief](SSO_INTEGRATION_DECISION.md). Security review points for the adapter: **(1) assertion validation** (anti-XXE / anti-signature-wrapping / reject-unsigned — the highest-risk surface) and **(2) JIT account-linking** (flagged separately below). Connection→tenant routing was **reviewed and is sound**: `initiateSso` takes a client-supplied `tenant` only to pick which IdP to redirect to — the IdP constrains *who* authenticates, the SSO txn (cookie-bound) carries the tenant, and the callback validates the assertion against *that* tenant's config; a client can't cross tenants. |
| **SSO JIT account-linking** (`sso/jit.ts`) — harden before enable | — | S | ◻ **flagged — security, before enable** | CONCRETE finding: `provisionSsoIdentity` auto-links an assertion to an existing GLOBAL user by `findByEmail` with **no domain check** (jit.ts:16). Harmless today (SSO stubbed in prod → not exploitable), but before a real adapter enables it, this is the takeover surface: a malicious/misconfigured ORG IdP asserting `victim@other-domain` would link to victim's global identity + join them to the attacker's org — and since the session rides the global identity, reach victim's OTHER orgs (org-switch checks membership, which now exists). **Mitigation to land WITH the real adapter:** only link/provision when the asserted email's domain is one of the org's VERIFIED domains (`auth_domains`, ADR-0017), and/or gate cross-provider linking on an explicit policy + `email_verified`. Flagged, not changed — the linking policy is a security-design call. |
