...
| 1.4c | Allowed-origins **repository + wiring**: `getScopeOrigins`/`upsertTenantOrigin` (mirrors effectivePolicyRepository); the redirect/CORS guards call `resolveAllowedOrigins(env floor, managed)`; the write path stores `canonicalManagedOrigin(input)` | M (AUTH-036) | ◻ next | Blind/DB + the `isAllowedOrigin` call-site switch (a live redirect-gate behaviour change → validate with a DB). |
