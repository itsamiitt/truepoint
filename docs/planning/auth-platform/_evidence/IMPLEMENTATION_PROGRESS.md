...
| Concurrent-session cap — **enforcement** | AUTH-042 | S | ◻ next | Wire the cap into `createSession`: resolve it from the engine (reuse `resolvePolicyFromRows`), and when the user is over the cap evict the OLDEST session(s). Fail-open (try/caught — a cap-read fault never breaks login); additive (unset cap = unlimited = today). |
