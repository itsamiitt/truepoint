...
| **WebAuthn / passkeys** — sign-in integration | AUTH-024 | M | ◻ next | The authentication routes (options/verify) + wiring passkeys into the login/MFA flow (a passkey as a first factor or an MFA method). The auth ceremony backend is ready. ⚠ touches the login flow — review before enable. |
