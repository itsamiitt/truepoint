...
| 1.2b | Write-path **DB methods**: `effectivePolicyRepository.setOrgKey` (withTenantTx, security_admin, audited) + `setPlatformKey` (withPlatformTx, staff-only, platform_audit_log) — upsert an `auth_policies` row after the 1.2a guard, bump the scope version | M (AUTH-021) | ◻ next | Blind/DB. Mirrors `authPolicyRepository.upsert`/`setEnforcement`. Needs the per-scope version-bump design (for 1.1b-cache invalidation). |
