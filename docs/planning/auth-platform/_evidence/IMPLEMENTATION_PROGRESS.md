...
| 1.2b | Write-path **DB upsert**: `effectivePolicyRepository.setOrgKey` (withTenantTx, security_admin, audited) + `setPlatformKey` (withPlatformTx, staff-only, platform_audit_log) — after the 1.2a value+floor guards pass, upsert the `auth_policies` row | M (AUTH-021) | ◻ next | Blind/DB — now a THIN upsert (validation is done + pure). Cache = **delete-on-write** (drop the resolver cache key), not row-versioning — simpler + correct (resolves the earlier version-bump open q). Watch the `onConflict` NULLS-NOT-DISTINCT target for the platform (NULL-tenant) upsert — DB-verify. |
