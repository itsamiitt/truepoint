...
| **WebAuthn / passkeys** — routes + UI | AUTH-024 | L | ◻ next | The WEBAUTHN_ENABLED-gated `/account/security/passkeys/*` routes (options/verify for register + authenticate) + the enroll/sign-in UI (`navigator.credentials.*`). App-layer; the ceremonies above are the backend. |
