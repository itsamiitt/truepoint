...
| `__Host-` refresh cookie — **WRITE flip + clear-both** | AUTH-074 | S | ◻ next | Flip the setters (`refreshCookie` string + `finishLogin` jar.set + the refresh routes) to `REFRESH_COOKIE_HOST` with `__Host-` attributes (Secure, Path=/, NO Domain); `clearRefreshCookie` clears BOTH names. Deploy AFTER the read stage is live everywhere. |
