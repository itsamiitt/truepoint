...
| **WebAuthn / passkeys** — enrollment UI | AUTH-024 | S | ✅ **done (gated)** | `PasskeySection.tsx` on the account-security page (added `@simplewebauthn/browser@13`): "Add a passkey" → `startRegistration` (handles the base64url encoding) → the options/verify routes; a cancelled prompt is a soft notice. Rendered ONLY when `WEBAUTHN_ENABLED` (page-gated) + the routes 404 when off → fully inert until enabled. typecheck (apps/auth) ✓ biome ✓. **Passkey ENROLLMENT is now complete end-to-end** (schema → store → repo → ceremony → routes → UI). |
