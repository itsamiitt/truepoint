...
| 1.1b-switch | Switch `flow.ts` `finalizeLogin` (+ refresh gates) to read the RESOLVED policy from the engine instead of `tenant_auth_policies` | part of L | ⏸ **deferred — needs a careful shadow-mode design** | Live login-enforcement change. Recommended rollout: read BOTH (old table + resolved engine), COMPARE + log discrepancies, ENFORCE with the OLD until validated, then cut over. `enforcement_enabled` stays on tenant_auth_policies (read alongside). Do design-first, not a blind flip. |
...
| 1.5b-rest | Remaining wiring: login **failure** metric (in `login.ts`, not finalizeLogin), the OTHER enforcement gates (`ip`/`sso`/`session`/`idle` — in ipAllowlist/sessionTimeout paths), `auth_mfa_challenge_total` in the MFA path, and a **`/metrics` scrape endpoint** (network-restricted) exposing `renderAuthMetrics()` per process (apps/api = revocation; apps/auth = login/token) | M | ◻ next | Endpoint is the value-unlock (makes token-mint + revocation + the new login SLIs scrapeable); needs the internal-only exposure decision (bearer token, off unless env set). |
