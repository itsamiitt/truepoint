...
| Quality dashboard (fill/bounce/conflict/freshness) live | ▶ net-new | `10 §5` (`22`/`11`) |
...
|---|---|---|---|
| 1 | **Verifier subsystem** (email hybrid Reacher+commercial; phone line-type) — unblocks "charge only for verified" already wired into `chargeFor` | `03`/`09 §5`; `01 §5.2/5.3` | MVP — **email + phone validation landed** (Reacher email verifier + `hybridVerifier`; Twilio Lookup phone verifier → carrier-confirmed valid/invalid; both config-gated + wired into reveal & reverify). Pending: commercial email secondary (vendor open); **carrier line-type (TCPA mobile/landline) needs a `phone_line_type` column → a drizzle migration, blocked in the no-bun sandbox** (run where `drizzle-kit` is available) |
