...
- **Status:** Accepted
- **Date:** 2026-06-17
- **Context doc:** [31-bulk-enrichment-pipeline.md](../31-bulk-enrichment-pipeline.md), [06-enrichment-engine.md](../06-enrichment-engine.md), [03-database-design.md](../03-database-design.md)
...
- **Sibling ADRs:** [ADR-0039](./ADR-0039-bulk-enrichment-pipeline.md) (bulk pipeline / ingest → match → enrich → export), [ADR-0038](./ADR-0038-bulk-enrichment-billing-forecast-and-quota.md) (bulk billing / quota — only `matched_provider` rows spend credits)

## Context

The bulk CSV enrichment product ([31](../31-bulk-enrichment-pipeline.md)) takes a sparse customer CSV
...
3. **Provider waterfall on residual misses only** (existing [06 §4](../06-enrichment-engine.md) waterfall via
   ...
   credits — [ADR-0038](./ADR-0038-bulk-enrichment-billing-forecast-and-quota.md)); a provider miss is `unmatched`.
...
infra lands). The bulk pipeline ([ADR-0039](./ADR-0039-bulk-enrichment-pipeline.md)) calls `MatchPort`; billing
([ADR-0038](./ADR-0038-bulk-enrichment-billing-forecast-and-quota.md)) reads the `match_outcome`. New milestone **M17**.
...
[31](../31-bulk-enrichment-pipeline.md) product promise and the [ADR-0038](./ADR-0038-bulk-enrichment-billing-forecast-and-quota.md)
unit economics.
...
| **Provider-first** (call the waterfall for every row) | Rejected | Slow (per-row network + rate limits) and expensive (every row burns provider spend even when we already own a verified golden record) — defeats the [31](../31-bulk-enrichment-pipeline.md) speed/cost promise. |
...
- **Positive:** internal matches are **free and instant**, so internally-answerable files finish in a fraction
  of a minute; provider spend is confined to true residual misses ([ADR-0038](./ADR-0038-bulk-enrichment-billing-forecast-and-quota.md));
