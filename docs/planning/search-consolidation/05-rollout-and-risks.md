...
| **2** | Global company search end-to-end: `MASTER_COMPANY_VISIBLE`, the two repositories, `0134`, contracts, `N3`/`N4`, `AccountsPane` merging owned + database rows, counts, sorting, pagination. **R1 must be resolved before this starts.** | `DATABASE_COMPANY_SEARCH_ENABLED` |
