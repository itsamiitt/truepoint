...
| **4 — DONE 2026-08-21** | Include/exclude on the global **people** contract (it had none, so `toDatabaseQuery` returned null for ANY excluding query and the database half silently vanished), the applied-filter chip row + Clear all on both tabs, the All / In-workspace / New-to-me control, `0135`, and the removal-cleanup sweep. Deviations: `TpChip` gained an optional `removeLabel` (a chip row announced eight identical "Remove" buttons); the workspace-scope control resolves into *which engine runs* rather than into a filter clause, so its two non-default modes query one population and their sort and count are exact. | — |
| **5 — DONE 2026-08-21** | Accounts CSV export (+ test) and `0136`: `title_function`, `career_started_on`, `primary_started_on` materialized on `master_persons`, recomputed by the landing in the same transaction that writes the stints. **Deviation — no new worker queue.** The one-time backfill runs inside `0136`; `masterPersonDerivedRepository` exposes bounded keyset helpers for an operator re-run after a bulk landing. Building a queue for a job that runs once was more machinery than the task. `EXPLAIN ANALYZE` is still **outstanding** — `0134`/`0135`/`0136` have not been applied to any database yet, so a plan captured now would show the pre-index shape. | — |
