...
> This doc evaluates whether LeadWolf's planned product actually closes the market gaps catalogued in
...
- **Our Coverage** — how well LeadWolf's *plan* addresses it: **Strong / Partial / Weak / None**, with the milestone that
...
> Competitor coverage and pain evidence cite the dossier inline. LeadWolf coverage is **projected** and milestone-tagged.
...
| **7. Contact-level (not account-only) intent / signal-based targeting** | **Partial.** ZoomInfo/Cognism intent is Bombora **account-level only**; signal-based selling is the winning trend but conversion-lift figures are *vendor estimates* [DevCommX](https://www.devcommx.com/blogs/signal-based-selling-vs-intent-data) *(flagged unverified)*. | **Partial (M4/M8).** ICP-fit + intent + engagement → composite 0–100 score (M4); deeper signals + activity timeline at M8. Intent *sourcing* depth is unproven. | **3** | **Medium.** Scoring layer is real; signal *data* depth depends on providers LeadWolf doesn't own. |
| **8. Deliverability discipline bundled with sending (warmup, SPF/DKIM/DMARC, complaint→suppression)** | **Partial/contested.** Cold-email cluster (Instantly/Smartlead) leads on infra but carries deliverability/lock-in complaints [TrulyInbox](https://www.trulyinbox.com/blog/instantly-reviews/); SEPs lack native warmup; AI-SDRs collapse domains. Google/Yahoo 2024 rules + Nov-2025 permanent rejections raise the bar [Mailgun](https://www.mailgun.com/state-of-email-deliverability/chapter/yahoogle-bulk-senders/). | **Partial — deferred (M9).** Send engine scopes warmup, auth, bounce/complaint→auto-suppression. *Not at MVP; LeadWolf should NOT out-volume Instantly/Smartlead on raw infra.* | **3** | **Deferred.** Compliance+deliverability *together* is the angle, but only lands at M9. |
...
- **Turnkey full loop (gap #5).** Find→reveal→score is MVP; **sequence+send is M9**, so the *complete* loop isn't turnkey at
  launch — at MVP LeadWolf is a clean data/reveal/score tool, not yet an end-to-end engine.
...
- **Scoring / contact-level signal (gap #7, M4/M8).** Composite ICP+intent+engagement score is real; intent *data depth*
  depends on providers LeadWolf doesn't own.
...
- **International & SMB long-tail coverage / fresh proprietary data (gap #13).** LeadWolf launches **US-only with no
  dataset of its own** — it cannot beat Apollo's EU gaps or Cognism's NA gaps because it has no independent data to do so.
- **A raw-volume / breadth play.** LeadWolf deliberately does **not** compete on database size (Apollo 275M, RocketReach
...
## 6. Unique differentiators (what only LeadWolf claims to combine)
...
| **"Compliant + affordable + full-loop" positioning** | Apollo owns "affordable + full-loop" (weak compliance); Cognism owns "compliant + premium + data-only." LeadWolf's intersection is genuinely vacant. | M1–M9 |

*All four are **projected** from `docs/planning/`. None is shipped or validated.*

## 7. Areas where competitors are STRONGER

| Area | Who | Why LeadWolf can't match it at/near launch |
|---|---|---|
...
| **Compliance *proof* (certs, broker registration, residency) today** | Cognism | LeadWolf has the *design* but none of the certs yet — see [Risk Assessment](07-risk-assessment.md). |
...
## 8. Areas where LeadWolf is STRONGER (projected)

| Area | vs whom | Basis |
|---|---|---|
| **Send-side + use-governing compliance** | Everyone | Only LeadWolf gates reveal *and* send in-transaction with DSAR fan-out + audit (M5). |
...
|---|---|---:|---:|---:|---|
| **Problem severity** | Are the gaps LeadWolf targets real, acute, and evidenced? | 25% | 85 | 21.25 | Compliance pressure (CA DROP, GM $12.75M, ZoomInfo ~$29.5M), tool-sprawl (70% overwhelmed, Gartner), accuracy/billing complaints are loud and well-sourced. Top-tier severity. |
...
*Sources: the LeadWolf dossier (inline-linked above) and its adversarial verification block — contested/unverified figures
(e.g. signal-conversion lifts, the 11x ARR figure corrected to ~$10M, third-party accuracy %s) are caveated inline.
LeadWolf remains **pre-launch with zero code/users**; all fit and score judgements are **PROJECTED from
