...
> **Stage caveat:** TruePoint is **pre-launch with zero code, users, or revenue**; all pricing is placeholder.
> Every claim that TruePoint "addresses" a gap is a **design intention from the planning corpus, not a shipped or measured capability.**
...
which ones TruePoint's plan targets and which are **hidden** opportunities the field has largely missed.

The single biggest finding: the loudest pains in this market are **not "we need more contacts."** The incumbents already have
hundreds of millions of records. The pains are **trust** (bad data, bounce rates, [opaque billing](https://www.cleanlist.ai/blog/2026-03-19-zoominfo-pricing-guide), [auto-renewal traps](https://www.g2.com/discussions/sneaky-auto-renewal-clause-in-zoominfo-contract)), **compliance liability** (the buyer inherits it), **tool sprawl**, and **deliverability collapse**. TruePoint's three differentiators — end-to-end-in-one-app, compliance-as-a-feature, per-workspace ownership — map onto exactly these pains. The risk is that several are **MVP-incomplete** (the send engine and CRM sync are M9–M10) or depend on data TruePoint does not yet own.
...
| **Risk (1–5)** | Execution + competitive risk to TruePoint of betting here (5 = high risk: incumbents can copy, or TruePoint lacks the asset). |
| **Difficulty (1–5)** | How hard it is to actually solve well (5 = very hard / capital- or data-intensive). |

> Scores are **directional analyst judgement** grounded in the dossier, not measured. For TruePoint specifically, Risk and
> Difficulty are elevated wherever the capability is **post-MVP (M7–M11)** or depends on a proprietary data asset TruePoint
...
**Risk:** high for TruePoint — it has **no proprietary dataset** and relies on Apollo/ZoomInfo/Clearbit as sources, so it inherits their accuracy ceiling. **Difficulty:** high.

**TruePoint coverage:** **Partial / claimed.** "Verified-on-reveal" (verification at reveal time, per-import provenance) is a
...
but TruePoint verifies third-party data rather than owning a fresher source, so it can credibly beat *transparency* but not necessarily *raw coverage*.
...
**TruePoint coverage:** **Planned, post-MVP (M9).** The send engine bundles suppression-gated send + CAN-SPAM footer + bounce/complaint→auto-suppression
([ADR-0009](../planning/decisions/ADR-0009-outreach-engine-enroll-and-send.md)). But this is **not in the MVP (M1–M5)**, and the dossier explicitly warns TruePoint should **not** try to out-build Instantly/Smartlead on raw deliverability infra.
...
**TruePoint coverage:** **Yes, MVP (M4).** Versioned composite 0–100 score is a core MVP capability
...
**TruePoint coverage:** **Core thesis — partially MVP.** "End-to-end in one app" is differentiator #1, but the *full* loop
...
**Why it exists.** Most low-end buyers don't compare TruePoint to a named tool — they compare it to **Sales Navigator + a
...
**TruePoint coverage:** **Yes, MVP.** One-click verified export + CRM sync (sync is M10), saved/re-runnable searches, and ingest of
...
**TruePoint coverage:** **Yes, MVP — a genuine structural moat.** Per-workspace ownership with hard Postgres RLS isolation is
...
**TruePoint coverage:** **Yes, by design.** The lean 6-destination single-page, panel-driven UX is built to answer exactly the
...
**TruePoint coverage:** **Yes.** Credits are a top-bar utility (not a paywall maze), first-reveal-wins per workspace (re-reveal
...
**TruePoint coverage:** **Yes, intended.** Transparent, self-serve, credit-based pricing is a stated wedge — **caveat: all
TruePoint pricing is placeholder**, so this is a *commitment*, not a validated price ([Product-Market Fit](04-product-market-fit.md)).
...
**TruePoint coverage:** **Yes, intended.** No-auto-renewal-trap, easy self-serve cancellation, and per-workspace data
...
**TruePoint coverage:** **Yes.** A single tenant-level credit pool is cleaner than Clay's two-meter model — though TruePoint must
...
**TruePoint coverage:** **Partial / post-MVP.** Public REST API + webhooks and HubSpot/Salesforce/Pipedrive sync are **M10**, not
...
**TruePoint coverage:** **Planned, post-MVP (M9).** Bounce/complaint→suppression and CAN-SPAM enforcement are scoped, but full
...
**TruePoint coverage:** **Yes (design) — differentiator #2.** Unbypassable suppression gating **both reveal and send** inside the
...
**TruePoint coverage:** **Not a documented product capability** — a go-to-market/ops commitment, not in the planning corpus. Flag as an **open execution gap** for TruePoint to own deliberately.
...
**TruePoint coverage:** **Partial.** Append-only platform audit log and per-workspace audit trail support an honest-metrics
...
> 5 = strongest. **Risk** and **Difficulty** are scored *for TruePoint specifically* (elevated where the capability is post-MVP
> or depends on an asset TruePoint lacks). "LW?" = does TruePoint's *plan* address it — ✅ MVP (M1–M5), 🔶 planned post-MVP
...
\* *5.1 is scored on the **intent**; TruePoint's actual prices are placeholder and unvalidated (pre-launch).*

**Reading the board.** The highest-leverage, lowest-difficulty wins cluster in **Pricing** (5.1, 5.2) — they are pure
policy/trust moves TruePoint can deliver cheaply at launch, against the market's angriest complaints. The highest-*demand*
functional/operational gaps (2.1 data trust, 3.1 consolidation) are also the **hardest and riskiest** for TruePoint because it
...
## 9. Where TruePoint's plan lands — coverage summary
...
| **Only partially solvable (no owned asset)** | 2.1 data accuracy — TruePoint verifies third-party data, it doesn't own a fresher source |
...
The pattern is clear: TruePoint's plan **best covers the trust, fairness, isolation, and UX gaps** (cheap, defensible, MVP-ready)
...
1. **Compliance gating of the customer's *own outbound* — not just clean source data.** Every "compliance" vendor (Cognism
   ...
   "we govern *your* compliant use end-to-end" is a structurally unmet need. **This is TruePoint's single most distinctive, lowest-competition opening** — provided the send-side (M9) and certs land.
...
2. **A "switching-cost-free / anti-lock-in" brand position.** ZoomInfo's data-destroy clause and auto-renewal trap are so
   ...
   credibly claim. Per-workspace data ownership makes this *structurally* true for TruePoint, not just a promise.
...
4. **The "augmented human, not autonomous slop" position.** The AI-SDR collapse (50–70% 90-day churn; hybrid 1-human+2-AI pods
   ...
   human-in-the-loop is now a **strength, not a limitation** — but only if TruePoint matches the autonomous players on
...
> **Caveat on the hidden opportunities.** Several depend on capabilities TruePoint does **not have at MVP** — the compliant send
...
(themselves cited to the underlying market, regulatory, and review sources linked inline above), and in the TruePoint planning
corpus under `docs/planning/` for all "TruePoint coverage" claims. Where a figure was flagged **contested or unverified** in the
verification block, that caveat is stated inline. TruePoint remains **pre-launch with zero code/users**; pricing is placeholder
