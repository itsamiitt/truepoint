...
> **Stage caveat:** TruePoint is **pre-launch with zero code, users, or revenue**; all pricing is placeholder. Every "TruePoint's
...
meeting — and asks, at each stage: *what hurts, how does today's market answer it, what does TruePoint's plan answer with, and what
pain still remains even if TruePoint executes perfectly?*
...
(the late stages where the inbox and the buyer punish bad outbound). TruePoint's plan lands cleanly on the *trust* and *economics*
...
and reporting pains sit in **post-MVP territory (M7–M11)**, so for an MVP-stage TruePoint the back half of the journey is mostly an
*intention*, not a capability. Crucially, TruePoint owns **no proprietary dataset** (it verifies third-party data from
...
| **TruePoint's Solution** | The documented plan response, tagged **✅ MVP (M1–M5)** / **🔶 post-MVP (M7–M11)** / **⚪ not a documented capability**. |
| **Remaining Unresolved Pain** | What still hurts *even if TruePoint ships its plan* — the honest residual. |
...
> ([Roadmap](../planning/10-roadmap.md)). This tag is load-bearing: a pain "addressed by TruePoint" at M9 is **not** addressed by an
> MVP-stage TruePoint.
...
This compresses the eleven stages into a single heat read: **how bad is the pain**, **does TruePoint win or lose there**, and **when**
(MVP vs post-MVP). "Friction" = how loud/frequent/emotional the cross-vendor complaint is in the dossier; "TruePoint verdict" is the
**projected** position assuming the plan executes.

| Journey stage | Friction (where it hurts) | Who feels it most | TruePoint verdict | When it lands |
|---|:--:|---|---|:--:|
...
### 3.1 Where friction is worst — and how TruePoint fares

Three stages carry the market's **loudest, most emotional, most cross-vendor pain**, and they happen to be exactly where TruePoint's
plan is *cheapest to win*:

- **Stage 3 (reveal/credit economics)** and **Stage 5 (list management)** are TruePoint's **cleanest projected wins.** They are
  architecture and policy choices — first-reveal-wins, don't-charge-for-bad-data, single credit meter, per-workspace ownership,
  anti-lock-in — that TruePoint can deliver **at MVP** against the angriest complaints (Apollo/Lusha credit gouging, Seamless
  pay-for-nothing, ZoomInfo's data-destroy clause). Low difficulty, high emotional payoff.
- **Stage 4 (compliance/risk)** is TruePoint's **most distinctive but only half-delivered** position. Gating the *customer's own
  outbound* — not just clean source data — is a genuinely unmet need that even Cognism (which doesn't send) structurally can't
  claim. But TruePoint gates **reveal at MVP and send only at M9**, and **SOC 2/ISO/DPA/EU-residency are open**, so the full moat
  is a future state.

### 3.2 Where TruePoint still leaves gaps

- **Stage 2 (data accuracy)** is the market's **single worst pain and TruePoint's structural soft spot**: with no proprietary
  dataset, it verifies third-party data and inherits its providers' accuracy ceiling. TruePoint can win the *trust framing*
...
- **Stages 7–10 (sequencing → deliverability → reply handling → CRM sync)** are the **back half of the journey and almost entirely
  post-MVP (M9–M10).** For an MVP-stage TruePoint, the celebrated "end-to-end in one app" differentiator is **find→reveal→score**,
  not **→sequence→send**. The deliverability stage is additionally a battleground the dossier says TruePoint should *not* fight on
  raw infra.
- **Stage 1 (discovery)** is a partial loss on raw coverage — **3 providers vs Clay's 100+** — so TruePoint must win on *workflow
...
> (deliverability, reply handling, CRM sync), milder in the middle (list management, scoring). TruePoint's plan is **strongest
...
> honest pre-launch summary: TruePoint can credibly win a buyer's *first impression and procurement trust* on day one, but cannot
...
the underlying market, regulatory, and review sources linked inline above), and in the TruePoint planning corpus under
`docs/planning/` for every "TruePoint's Solution" claim. Where a figure was flagged **contested or unverified** in the verification
block, that caveat is stated inline. TruePoint remains **pre-launch with zero code/users**; pricing is placeholder and all
