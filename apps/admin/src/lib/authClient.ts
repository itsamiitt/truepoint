// authClient.ts — the staff-console token client (ADR-0016). A thin instantiation of @leadwolf/auth-client:
...
//
// The platform-admin gate (adminGate.ts) is layered on top: a valid token is necessary but NOT sufficient —
// the api `/admin/*` surface re-checks the signed `pa` claim and 403s a non-staff caller (ADR-0034).
//
