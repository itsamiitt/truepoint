// authClient.ts — the Forge operator-console token client (ADR-0016). Mirrors apps/admin + apps/web: the access
// token lives IN MEMORY only (never localStorage, never a cookie). It starts login via a PKCE redirect to the
// auth origin, exchanges the returned code, and silently refreshes against auth.*/token/refresh (same-site
// credentialed fetch — the refresh cookie stays on the auth origin) shortly before expiry. The forge gate
// (forgeGate.ts) is layered on top: a valid token is necessary but NOT sufficient — the forge-api `/bff/*`
// surface re-checks staff authorization and 403s a non-staff caller.
...
const PKCE_VERIFIER_KEY = "lw_forge_pkce_verifier";
const STATE_KEY = "lw_forge_oauth_state";

/** One-shot flag the callback sets before auto-restarting login, so recovery happens AT MOST once. */
export const RECOVERY_KEY = "lw_forge_auth_recovery";
...
/**
 * End the operator session: revoke + clear the refresh cookie on the auth origin (idempotent), drop the
