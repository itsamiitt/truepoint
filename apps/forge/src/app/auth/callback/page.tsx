// page.tsx — forge.truepoint.in/auth/callback: receives the single-use code from the auth origin, validates the
// round-tripped state, exchanges the code for an in-memory access token (ADR-0016), then goes straight to the
// work surface via a CLIENT-SIDE router navigation (not window.location.replace). A full-document reload would
// tear down the JS context and DISCARD the just-minted in-memory token, forcing the shell into a second,
// redundant silent refresh — and would still need the extra "/" → "/overview" hop. A client nav keeps the token
// alive and lands directly on the destination, so the shell renders signed-in with zero extra round-trips.
...
// single-use state) we auto-restart a fresh login exactly once — guarded by a sessionStorage flag that survives
// the auth round-trip — so a one-time stale state is no longer a dead end. Otherwise we log the real reason to
// the console (DevTools) and map it to a user-facing message — a server fault asks the user to wait.
"use client";
...
// The default signed-in destination (Overview). Navigating here directly from the callback collapses the old
// callback → "/" → "/overview" chain into a single hop.
