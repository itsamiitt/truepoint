...
import { Alert, Button, Card } from "@leadwolf/ui";
...
export default async function EnrollPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (result.kind === "totp") {
    ...
    return (
      <AccountShell title="Set up authenticator app" sections={[]}>
        {/* Same DS Card as AccountSectionCard, for the same reasons — this screen is reached FROM the account
            page, so a hand-rolled surface here would be a visibly different card one click later. Card takes
            no aria props, hence the labelled <section> wrapper; the white fill is the one override (Card's
            default --tp-surface-2 IS this origin's page background). The 17px headings below are off the
            --tp-text-* ladder deliberately, matching AccountSectionCard's card heading. */}
        <section aria-labelledby="enroll-totp-heading">
          <Card as="div" style={{ background: "var(--tp-surface)" }}>
            ...
            >
              In your authenticator app, add an account using this setup key (or the link below).
              Keep it secret — anyone with this key can generate your codes.
            </p>
            <dl
              style={{
  ...
  return (
    <AccountShell title="Save your recovery codes" sections={[]}>
      <section aria-labelledby="enroll-recovery-heading">
        <Card as="div" style={{ background: "var(--tp-surface)" }}>
          <h2
          ...
          <Alert variant="default" role="status" className={styles.spaced}>
            These codes are shown once. Store them somewhere safe — each one signs you in if you
            lose your authenticator, and works only once.
          </Alert>
          <ul
            style={{
              marginBottom: "var(--tp-space-5)",
