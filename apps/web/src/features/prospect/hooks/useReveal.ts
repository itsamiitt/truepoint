...
// insufficient_credits (402) vs suppressed (403). On success it invalidates the shared credit-balance key so
// every surface showing the balance re-reads it. Holds no business logic — the charge/gate run server-side
// in packages/core.
"use client";
  ...
  const run = useCallback(
    async (id: string, revealType: RevealType): Promise<RevealResponse | null> => {
      ...
      try {
        ...
        setResult(res);
        // Credits moved — invalidate so every surface reading the balance re-reads it.
