...
soakDescribe("S-P4 nightly soak — TP-3 (concurrent-tenant fairness)", () => {
  test(`TP-3a: fast-lane p95 ≤ ${SOAK_FAST_P95_MS}ms while a ${SOAK_WHALE_ROWS}-row whale runs; whale in-flight ≤ K=${K}`, async () => {
  ...
  test("TP-3b: two whales under K=1 windows interleave chunk-for-chunk (neither starves the other)", async () => {
    const whaleScope = { tenantId: whaleTenant, workspaceId: whaleWs };
