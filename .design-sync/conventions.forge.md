...
**The primitives need no provider** — everything in the `general` group renders standalone, and so do the Forge surfaces.

## The operator console (this project's app groups)

Alongside the primitives this bundle ships the **real Forge operator console** — 7 components from the product itself, not recreations.

- **Chrome**: `ForgeShell` frames a page (rail badged "Forge console", top bar, command palette). It runs a two-stage authn/authz gate and renders its own redirecting / forbidden / error states.
- **The six boards**, each taking no props and loading its own data: `OverviewPage` (KPI tiles + recent captures), `CapturesPage` (the capture feed), `ParsersPage` (the parser registry), `ReviewPage` (the human-review queue), `SourceFetchesPage` (the URL fetch registry), `SyncStatusPage` (downstream sync health).

**Design rules this console encodes.** Every board is **read-only** — the console never writes to a database. The source-fetch registry holds **URLs and outcomes only, no PII**; don't add a name or email column to it. Health and status are always shown as a `StatusBadge` tone plus the raw number, so an operator can see both the judgement and the evidence.

**Page scaffold.** Console pages use `.tp-page`, `.tp-stat-grid` (the KPI row), `.tp-section-title` and `.tp-cell-mono` from the shared app-shell stylesheet, which `styles.css` already imports. Use them for page-level layout rather than re-inventing a grid.
