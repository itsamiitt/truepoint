...
| [`P1-01`](P1-01-auth-policy-enforcement.md) | Auth-policy **enforcement** on login (IP allowlist + session timeout) | P1 | **Partial — kill-switch, run gates** | Gates C (IP allowlist) + D (absolute session timeout) enforce at finalizeLogin/refresh, **all behind `AUTH_POLICY_ENFORCEMENT_ENABLED` (default OFF = no-op)**; CIDR util + unit tests; reviewed (OFF-by-default proven byte-for-byte). **Deferred:** allowed-methods (needs txn-method threading), forced-MFA-enroll UI, per-tenant flags, idle-timeout, require_sso. |
