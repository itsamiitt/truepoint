...
| [`P1-01`](P1-01-auth-policy-enforcement.md) | Auth-policy **enforcement** on login (IP allowlist, session timeout, allowed-methods, forced MFA enrollment) | P1 | **Code written — run gates** | Gates B (allowed-methods, via a txn `method` set at the 4 edges) + C (IP allowlist) + D (session timeout) + sub-gate A (forced in-login MFA enrollment → `/mfa/enroll`); CIDR + cap-math unit tests; OFF-by-default proven byte-for-byte; the `mfa_required` backstop unchanged when OFF. **P1-01 completion increment (this branch):** enforcement is now gated on a **per-tenant** switch (`tenant_auth_policies.enforcement_enabled`, staff-set default OFF) AND-ed with the `AUTH_POLICY_ENFORCEMENT_ENABLED` master-arm, with a **break-glass disable** at `POST /api/v1/admin/tenants/:id/auth-enforcement` (super_admin, audited); **idle timeout** wired on the refresh path (`isIdleExpired` + `lastSeenAt`, idle±1s tests); **`mfa.enroll` audit** added to the enum + emitted at both enroll paths; `authPolicyEnforcement.itest.ts` proves default-OFF / preserve-on-tenant-edit / break-glass / audit-per-write. **Still deferred:** `require_sso` enforcement (waits for the P2 SSO adapters); the `apps/admin` console UI for the staff toggle. |
