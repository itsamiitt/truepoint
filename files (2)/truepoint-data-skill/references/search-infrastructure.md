...
- **The search engine** (OpenSearch/Elasticsearch baseline; Typesense/Vespa
  acceptable for the workload) holds a **derived, query-optimised projection** of
  that data — the surface that answers faceted/full-text queries fast.
- They are **never two independent sources**. The index is rebuilt from Postgres;
  if they disagree, Postgres wins and the index is corrected. A write goes to
  Postgres first, then propagates to the index — never to the index alone.
...
- **Tenant scope**: search over a tenant's working set (their Prospects) is filtered
  by `org_id`. Search over the shared canonical dataset (prospecting new people)
  returns canonical records, but what an org can *pull into their CRM* and the
  per-org overlay (already-added, owner, list membership) is tenant-scoped.
- **Visibility scope**: results a user sees within their org respect ownership/
  sharing (see `ownership-and-sharing.md`) — enforced by filtering the query and
  re-checking on access, never by hiding results client-side.
- The index must carry the fields needed to apply these filters (e.g. `org_id`,
  `owner_id` on tenant-projected documents) so scoping happens in the query, not
  after.
