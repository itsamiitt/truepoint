...
- **Per-tenant quotas/credits** — an org can't exceed its plan's enrichment
  allowance; a stolen session can't run up an unbounded bill.
- **Rate-limited** per user and per org server-side.
- **Bulk enrichment** is a job with per-tenant fairness so one org's
