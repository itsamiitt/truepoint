...
- **Deleting a tenant's Contact/Prospect** removes *that org's working record and
  relationship* — their notes, ownership, list memberships, deal links. It does
  **not** necessarily remove the canonical Person, which other orgs may reference.
...
- **Right of access (DSAR)**: a person/org can obtain the data held about them. The
  export feature (architecture dependency-wiring) doubles as this mechanism — and
  is therefore itself **tenant-scoped, access-controlled, and rate-limited** (an
  unscoped export is a bulk leak — see security data-protection).
