...
- **Mailbox check (SMTP probe)** — confirming the specific mailbox exists, where
  ...
  - When using a third-party verification provider, the same outbound-safety, key-
    ...
    integrations).
- **Result is a graded verdict**, not just true/false: valid, invalid, risky
  (catch-all/accept-all domains where the mailbox can't be confirmed), unknown.
  The UI and sending logic treat "risky/unknown" differently from "valid" — don't
  collapse the grades.
...
- Does email verification escalate syntax → MX → mailbox, stopping at a confident
  verdict, and return a graded result (valid/invalid/risky/unknown)?
