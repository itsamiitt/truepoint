...
- **Postgres is the source of truth; the index is the query surface.** The
  large prospect/company dataset is *stored* in Postgres and *searched* via the
  search engine. They are kept in sync by a pipeline; they are never two
...
- **Visibility defaults to the owner; sharing is explicit.** A record is visible
  to its owner within the org by default; broader visibility is granted explicitly
  (see `ownership-and-sharing.md`).
