...
**`Guidelines/`** — the canonical brand kit folder in the codebase root. This is
the primary source of truth. It contains the logo files, icon exports, colour
definitions, typography specimens, spacing rules, usage guidelines, and any other
brand documentation the design team maintains. Before making any brand-level
decision — logo placement, colour use, typography, iconography — read the relevant
file in `Guidelines/` first.

**`@truepoint/ui`** — the code implementation of the brand in the design system.
...
> Earlier material located these in a single `@truepoint/ui` file. The real home is the
> `@truepoint/ui` package (imported normally — see the design skill SKILL.md). Where
> this file refers to "the implementation in `@truepoint/ui`", read the corresponding
> `@truepoint/ui` component/token. The brand is implemented **once** in the design
> system and imported, never re-drawn per surface or per repo.
...
3. Cross-reference `@truepoint/ui` for the established code pattern
...
The following is how brand elements are implemented in `@truepoint/ui`.
...
All icons in `@truepoint/ui` follow: `viewBox="0 0 24 24"`, `fill="none"`,
...
at the top of `@truepoint/ui` with the `I` prefix, matching the same stroke
