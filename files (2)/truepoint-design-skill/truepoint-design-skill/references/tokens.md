...
The lint config will flag raw hex and px values.

> **Tokens are a single shared source across both repos.** The customer and internal
> design systems may diverge in *components*, but they consume **one** token set —
> the brand does not fork between surfaces (see **truepoint-architecture**
> shared-packages). A new token is added to the shared source, not redefined per
> repo. (References to `@truepoint/ui`/`CLAUDE.md` below are from the prototype; the
> tokens live in the shared `@truepoint/ui` token source.)
...
ScorePill thresholds (from `@truepoint/ui`):
```js
