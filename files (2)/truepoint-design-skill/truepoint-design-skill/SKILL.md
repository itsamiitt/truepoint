...
TruePoint's UI is built on `@truepoint/ui` — the shared React component and token
system, consumed as a normal package import in both frontend repos. Every UI
decision flows from this design system. This skill makes sure every agent reaches
...
> `@leadwolf/ui` loaded via a `window.TruePointUI` global inside a single
> `crm-app.jsx` file. That was a prototype. The design system is **`@truepoint/ui`**,
...
> `@leadwolf/ui`, `window.TruePointUI`, or "source of truth in `crm-app.jsx`", read
> the `@truepoint/ui` package as the real source. This corrects the naming and the
> access pattern throughout.
...
2. **Shared atoms** — Does `Sidebar`, `Topbar`, `ContactDrawer`, `SmartSearch`,
   `ScorePill`, or `FilterBar` already exist in `@truepoint/ui` (or the app's shared
...
`@truepoint/ui` is imported like any package — there is no window global and no
load-order dance:

```jsx
// ✅ normal import
import { Card, TpButton, DataTable } from '@truepoint/ui'
...
This resolves the prior Tailwind-vs-inline contradiction. One approach:

- **Components encapsulate their own styling.** Reach for a `@truepoint/ui`
  component before styling a `<div>`; the look lives in the DS.
- **App-level layout uses inline `style={{ }}` reading `var(--tp-*)` tokens** — not
  Tailwind utility classes in app JSX, and not raw values.
...
The only `<style>` exceptions: `@keyframes`, `@font-face`, and the scrollbar styling
already defined in the shell. (How `@truepoint/ui` styles its *own* internals is the
package's concern; app code composes components + token-driven inline styles.)
...
```jsx
import { StateSwitch, LoadingState, EmptyState, ErrorState } from '@truepoint/ui'
...
- **No Tailwind utility classes in app JSX / no class-based CSS** — token-driven
  inline styles.
